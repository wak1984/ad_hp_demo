...
enum class RecordingEventType : unsigned {
    RECORDING_EVENT_UNKNOWN = 0,
    RECORDING_EVENT_START = RECORDING_EVENT_UNKNOWN + 1,
    RECORDING_EVENT_JOIN = RECORDING_EVENT_START,
    RECORDING_EVENT_LEAVE = 2,
...
enum RecordingEventKeyIndex {
    ...
    RECORDING_JOIN_EVT_AUDIO_PROFILE = 19,
    RECORDING_JOIN_EVT_AVSYNC_TYPE = 20,
...
#endif
#if defined(FEATURE_CONFIG_SERVICE_TEST)
    virtual void onReceivedConfiguration(const char* config) {
        (void)config;
    }
    virtual void onReceivedABTestConfiguration(const char* config) {
        (void)config;
    }
#endif
};
...
public:
    ...
    */
    ...
    virtual int registerAvTransportPacketInfoObserver(IRtcAvTransportPacketInfoObserver* oberver) = 0;
