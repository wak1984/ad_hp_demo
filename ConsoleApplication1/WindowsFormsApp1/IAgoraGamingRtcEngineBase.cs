...
*/
    ...
    */
    ...
    public abstract class IRtcEngineBase
    {
        ...
        [DllImport(MyLibName, CharSet = CharSet.Ansi)]
        public static extern int startScreenCapture(int captureFreq, int rc_left, int rc_top, int rc_width, int rc_height, int bitrate, uint processID);

        [DllImport(MyLibName, CharSet = CharSet.Ansi)]
        public static extern int stopScreenCapture();

        [DllImport(MyLibName, CharSet = CharSet.Ansi)]
        public static extern int startScreenCaptureEx(IntPtr windowId, int captureFreq, int rc_left, int rc_top, int rc_width, int rc_height, int bitrate);

        [DllImport(MyLibName, CharSet = CharSet.Ansi)]
        public static extern int stopScreenCaptureEx();

        [DllImport(MyLibName, CharSet = CharSet.Ansi)]
        public static extern int setExternalAudioSink(bool enabled, int sampleRate, int channels);
        
        [DllImport(MyLibName, CharSet = CharSet.Ansi)]
        public static extern int pushExternalAudioFrame(int type, int samples, int bytesPerSample, int channels, int samplesPerSec, byte[] buffer, Int64 renderTimeMs, int avsync_type);
        
        [DllImport(MyLibName, CharSet = CharSet.Ansi)]
        protected static extern void freeObject(IntPtr obj);

        [DllImport(MyLibName, CharSet = CharSet.Ansi)]
        protected static extern int setupLocalVideo(int view, int renderMode, UInt32 uid, IntPtr priv);
        
        [DllImport(MyLibName, CharSet = CharSet.Ansi)]
        protected static extern int setupRemoteVideo(int view, int renderMode, UInt32 uid, IntPtr priv);
        ...
        protected delegate void EngineEventOnCaptureVideoFrameRawHandler(int width, int height, IntPtr yBuffer, int rotation);
        ...
        [DllImport(MyLibName, CharSet = CharSet.Ansi)]
        protected static extern void initEventOnCaptureVideoFrameRaw(EngineEventOnCaptureVideoFrameRawHandler onCaptureVideoFrameRaw);
